...
const char *get_m1_reg_name(uint32_t addr) {
  ...
  static const char *tdma_src_names[] = {
      "DMAConfig",   "pad0",  "BaseAddr",     "RowStride",    "PlaneStride",  "DepthStride",
      "GroupStride", "pad1",  "pad2",         "pad3",         "pad4",         "pad5",
      "pad6",        "pad7",  "Fmt",          "pad8",         "pad9",         "pad10",
      "pad11",       "pad12", "PixelOffset0", "PixelOffset1", "PixelOffset2", "PixelOffset3"};
  static const char *tdma_dst_names[] = {"DMAConfig",   "BaseAddr",    "RowStride", "PlaneStride",
                                         "DepthStride", "GroupStride", "Fmt"};
  static const char *kdma_names[] = {"Unknown", "Unknown", "CoeffDMAConfig", "CoeffBaseAddr",
                                     "CoeffBfrSize"};
...
const char *get_m4_reg_name(uint32_t addr) {
  static const char *common_names[] = {
      ...
      "UnicastCfg", "TileHeight",  "TileOverlap", "MacCfg",     "NECfg",   "PatchCfg",
      "PECfg",      "NID",         "DPE",         "DPE0",       "DPE1"};
  ...
  static const char *tdma_src_names[81] = {"Src1DMAConfig",
                                           ...
                                           "pad_79",
                                           "pad_80"};
  static const char *tdma_dst_names[] = {"DstDMAConfig",
                                         "pad0",
                                         "DstBaseAddrLo",
                                         "DstBaseAddrHi",
                                         "DstRowStride",
...
const char *get_h17_reg_name(uint32_t addr) {
  // Common names same as M4
  static const char *common_names[] = {
      "ChannelCfg", "InWidth",     "InHeight",    "InChannels", "InDepth", "OutWidth",
      "OutHeight",  "OutChannels", "OutDepth",    "NumGroups",  "ConvCfg", "ConvCfg3d",
      "UnicastCfg", "TileHeight",  "TileOverlap", "MacCfg",     "NECfg",   "PatchCfg",
      "PECfg",      "NID",         "DPE",         "DPE0",       "DPE1"};
  // L2 names same as M4 + 1
  static const char *l2_names[] = {"L2_Control",        "L2_Src1Cfg",        "L2_Src2Cfg",
                                   "L2_SrcIdxCfg",      "L2_Src1Base",       "L2_Src1CStride",
                                   "L2_Src1RStride",    "L2_Src1DStride",    "L2_Src1GStride",
                                   "L2_Src2Base",       "L2_Src2CStride",    "L2_Src2RStride",
  ...
  // Need to define expanded arrays for TileDMA and KernelDMA in H17
  static const char *tdma_src_names[83] = {"Src1DMAConfig",
                                           "Src2DMAConfig",
                                           "Src1WrapCfg",
                                           "Src2WrapCfg",
                                           "Src1BaseAddrLo",
  ...
  static const char *tdma_dst_names[23] = {
  ...
  static const char *kdma_names[74] = {"MasterCfg",
                                       "AlignedCoeffSize",
                                       "Prefetch",
                                       "Res_0",
                                       "Res_1",
  ...
  static const hwx_reg_range_t h17_ranges[] = {
      {H16_COMMON_START, H17_COMMON_COUNT, common_names},
      {H16_L2_START, H17_L2_COUNT, l2_names},
      {H16_PE_START, H17_PE_COUNT, pe_names},
      {H16_NE_START, H17_NE_COUNT, ne_names},
      {H16_TILEDMA_SRC_START, H17_TILEDMA_SRC_COUNT, tdma_src_names},
      {H16_TILEDMA_DST_START, H17_TILEDMA_DST_COUNT, tdma_dst_names},
      {H16_KERNELDMA_START, H17_KERNELDMA_COUNT, kdma_names},
      {H16_CACHEDMA_START, H17_CACHEDMA_COUNT, cdma_names},
  };
...
const char *get_h18_reg_name(uint32_t addr) {
  // Similar to H17 but with H18 counts
  static const char *common_names[] = {
      "ChannelCfg", "InWidth",     "InHeight",    "InChannels", "InDepth", "OutWidth",
      "OutHeight",  "OutChannels", "OutDepth",    "NumGroups",  "ConvCfg", "ConvCfg3d",
      "UnicastCfg", "TileHeight",  "TileOverlap", "MacCfg",     "NECfg",   "PatchCfg",
  ...
  static const char *tdma_dst_names[27] = {
      "DstDMAConfig",      "pad0",           "DstBaseAddrLo",     "DstBaseAddrHi",
      "DstRowStride",      "DstPlaneStride", "DstDepthStride",    "DstGroupStride",
      "DstInternalCfg",    "pad1",           "DstMetaDataAddrLo", "DstMetaDataAddrHi",
      "DstFmtMode",        "pad2",           "DstFmtCtrl",        "pad3",
  ...
  static const char *kdma_names[83] = {"MasterCfg",
                                       "AlignedCoeffSize",
                                       "Prefetch",
                                       "Res_0",
                                       "Res_1",
  ...
  static const hwx_reg_range_t h18_ranges[] = {
      {H16_COMMON_START, H18_COMMON_COUNT, common_names},
      {H16_L2_START, H18_L2_COUNT, l2_names},
      {H16_PE_START, H18_PE_COUNT, pe_names},
      {H16_NE_START, H18_NE_COUNT, ne_names},
      {H16_TILEDMA_SRC_START, H18_TILEDMA_SRC_COUNT, tdma_src_names},
      {H16_TILEDMA_DST_START, H18_TILEDMA_DST_COUNT, tdma_dst_names},
      {H16_KERNELDMA_START, H18_KERNELDMA_COUNT, kdma_names},
      {H16_CACHEDMA_START, H18_CACHEDMA_COUNT, cdma_names},
  };

  return lookup_reg_name(addr, h18_ranges, 8);
}

void dump_hw_blocks(const hwx_state_t *state, const hwx_block_info_t *blocks, int count,
...
const char *get_hw_tensor_format_name_v17(uint32_t mode, uint32_t mem_fmt, uint32_t trunc,
                                          uint32_t shift) {
...
const char *get_pe_condition_name_v17(uint32_t cond) {
  static const char *labels[] = {"None",         "Abs",       "Equal", "Greater",
                                 "GreaterEqual", "LessEqual", "Less",  "NotEqual"};
...
void print_common_h13(const hwx_state_t *state) {
  ...
  const ane_common_h13_t *common = (const ane_common_h13_t *)&state->values[H13_COMMON_START / 4];
...
void print_l2_h13(const hwx_state_t *state) {
  ...
  printf("        L2 SourceCfg: Type=%u Dep=%u DMAFmt=%s Intrlv=%u CmpV=%u "
         "OffCh=%u\n",
         l2->scfg.type, l2->scfg.dep, get_l2_dma_fmt_name(l2->scfg.fmt), l2->scfg.interleave,
  ...
  printf("        L2 ResultCfg: Type=%u Bfr=%u DMAFmt=%s Intrlv=%u CmpV=%u "
         "OffCh=%u\n",
         l2->rcfg.type, l2->rcfg.bfrmode, get_l2_dma_fmt_name(l2->rcfg.fmt), l2->rcfg.interleave,
...
void print_common_h16(const hwx_state_t *state) {
  ...
  if (state->valid[(H16_COMMON_START + 0x04) / 4] || state->valid[(H16_COMMON_START + 0x08) / 4] ||
      state->valid[(H16_COMMON_START + 0x0C) / 4] || state->valid[(H16_COMMON_START + 0x10) / 4]) {
    printf("        InDim     : W=%u H=%u C=%u D=%u Type=%s (Src2Type=%s)\n", inw, inh, inc, ind,
           get_ch_fmt_name(infmt), get_ch_fmt_name(src2infmt));
  }

  if (state->valid[(H16_COMMON_START + 0x14) / 4] || state->valid[(H16_COMMON_START + 0x18) / 4] ||
      state->valid[(H16_COMMON_START + 0x1C) / 4] || state->valid[(H16_COMMON_START + 0x20) / 4]) {
    printf("        OutDim    : W=%u H=%u C=%u D=%u Type=%s\n", outw, outh, outc, outd,
           get_ch_fmt_name(outfmt));
  }
  ...
  if (state->valid[(H16_COMMON_START + 0x3C) / 4]) {
    printf("        MacCfg    : TaskType=%u %s ActiveNE=%u SmSrc=%u ReluType=%u "
           ...
           task_type,
           (task_type != 0)
               ? [[NSString stringWithFormat:@"(%s)", get_hw_task_type_name(task_type)] UTF8String]
               : "((None))",
...
void print_ne_h16(const hwx_state_t *state) {
  ...
  if (state->valid[H16_NE_START / 4]) {
    printf("        KernelCfg: Fmt=%s Pal=%d(%dbit) SparseEn=%d Reuse=%d", get_ch_fmt_name(kfmt),
           pen, pbits, sen, reuse);
    if (state->instr_ver >= 20) {
      printf(" SBS(W/A)=%d/%d\n", sbs_w, sbs_a);
    } else {
  ...
  if (state->valid[(H16_NE_START + 0x4) / 4]) {
    printf("        MacCfg: Op=%u (%s) KMode=%u BiasEn=%u PassEn=%u MVBiasEn=%u\n", op,
           get_ne_op_mode_name(op), km, bias_en, pass_en, mv_bias_en);
    printf("                BinPoint=%u PostEn=%u NLMode=%u MaxPoolEn=%u ArgSel=%u DblInt8=%u",
           bin_point, post_en, nl_mode_ne, max_pool_en, arg_sel, double_int8_en);
    if (state->instr_ver >= 20) printf(" SmallSrc=%u", ssrc);
    printf("\n");
  }

  if (state->valid[(H16_NE_START + 0x8) / 4]) printf("        MatrixBias: 0x%08x\n", mbias);
  if (state->valid[(H16_NE_START + 0x0c) / 4]) printf("        NEBias: 0x%08x\n", nebias);
  if (state->valid[(H16_NE_START + 0x10) / 4]) printf("        PostScale: 0x%08x\n", ps);
  if (state->valid[(H16_NE_START + 0x14) / 4])
    printf("        RcasConfig: KeyMask=0x%02x CmpBit=%u Axis=%u SenseBit=%u Mode=%u\n",
           rcas & 0xFF, (rcas >> 8) & 0x7, (rcas >> 12) & 0x3, (rcas >> 16) & 0xF,
           (rcas >> 20) & 0x1);
  ...
  if (state->valid[(H16_NE_START + 0x1c) / 4])
    printf("        SRSeeds: 0x%08x 0x%08x 0x%08x 0x%08x\n", seeds[0], seeds[1], seeds[2],
           seeds[3]);
  if (state->valid[(H16_NE_START + 0x2c) / 4]) printf("        QuantZeroPoint: %d\n", qzp);
}
void print_pe_h16(const hwx_state_t *state) {
  uint32_t task_type = 0;
  if (state->instr_ver >= 20) {
    task_type = ((const ane_common_h18_t *)&state->values[H16_COMMON_START / 4])->maccfg.task_type;
  } else if (state->instr_ver >= 19) {
    task_type = ((const ane_common_h17_t *)&state->values[H16_COMMON_START / 4])->maccfg.task_type;
  } else {
    task_type = ((const ane_common_h16_t *)&state->values[H16_COMMON_START / 4])->maccfg.task_type;
  }

  if (state->valid[(H16_COMMON_START + 0x3c) / 4] && task_type == 0) return;

  printf("        --- Planar Engine (0x4500) ---\n");

  uint32_t pool = 0, op = 0, lut_en = 0, cond = 0, red_idx = 0, red_keep = 0, nl = 0, src1 = 0,
           src2 = 0;
  ...
  if (task_type == 7) {
    ...
    printf("        PE Config (GOC) : Cond=%u CtoW=%u Src1Sel=%u Src2Sel=%u\n",
           (pe_common_cfg >> 4) & 0x1F, (pe_common_cfg >> 10) & 1, (pe_common_cfg >> 16) & 3,
           (pe_common_cfg >> 18) & 3);
...
void print_pe_h17(const hwx_state_t *state) {
  ...
  if (state->valid[H16_PE_START / 4]) {
    static const char *pe_op_names[] = {"None", "Add", "Mul", "Min", "Max", "5?", "6?", "7?"};
    printf("        PE Config : Pool=%u Op=%u(%s) LutEn=%u Cond=%u RedIdx=%u "
           "RedKeep=%u NLMode=%u CtoW=%u Src1Idx=%u Src2Idx=%u MaxIdx=%u\n",
           pe.pe_cfg.pool_mode, pe.pe_cfg.op, pe_op_names[pe.pe_cfg.op & 7], pe.pe_cfg.lut_en,
           pe.pe_cfg.cond, pe.pe_cfg.red_idx, pe.pe_cfg.red_keep, pe.pe_cfg.nl_mode,
           pe.pe_cfg.c_to_w, pe.pe_cfg.src1_idx, pe.pe_cfg.src2_idx, pe.pe_cfg.max_idx);
  }
  if (state->valid[(H16_PE_START + 0x4) / 4])
  ...
  if (state->valid[(H16_PE_START + 0x18) / 4]) {
    printf("        PE Src1   : Index=%u Relu=%d Transpose=%d\n", pe.src1_cfg.src1_idx,
           pe.src1_cfg.relu, pe.src1_cfg.transpose);
  }
  if (state->valid[(H16_PE_START + 0x20) / 4]) {
    printf("        PE Src2   : Index=%u Relu=%d Transpose=%d\n", pe.src2_cfg.src2_idx,
           pe.src2_cfg.relu, pe.src2_cfg.transpose);
  }
}
...
void print_l2_h17(const hwx_state_t *state) {
  const ane_l2_h17_t *l2 = (const ane_l2_h17_t *)&state->values[(H16_L2_START) / 4];
  ...
  printf("        L2_Src1: Base=0x%x0 RS=0x%x0 CS=0x%x0 DS=0x%x0 GS=0x%x0\n", l2->src1.base,
         l2->src1.row_stride, l2->src1.channel_stride, l2->src1.depth_stride,
  ...
  printf("        L2_Result: Base=0x%x0 CS=0x%x0 RS=0x%x0 DS=0x%x0 GS=0x%x0 Type=0x%x\n",
         ...
         l2->result.group_stride, l2->result.res_type_cfg);
...
void print_l2_h18(const hwx_state_t *state) {
  const ane_l2_h18_t *l2 = (const ane_l2_h18_t *)&state->values[(H16_L2_START) / 4];
...
void print_pe_index_h16(const hwx_state_t *state) {
  ...
  if (!state->valid[addr / 4]) return;
  ane_pe_index_h16_t idx = *(ane_pe_index_h16_t *)&state->values[addr / 4];
  printf("        --- PE Indexing ---\n");
  printf("        PE IndexCfg: MaxIndex=%d Enable=%d\n", idx.pe_index_cfg.max_index,
         idx.pe_index_cfg.indexing_en);
}

void print_l2_h16(const hwx_state_t *state) {
  const ane_l2_h16_t *l2 = (const ane_l2_h16_t *)&state->values[H16_L2_START / 4];
  ...
  static const char *l2_type_names[] = {"L2Read", "DmaRead2", "DmaRead", "L2ChainRead"};
#define L2_TYPE_STR(t) (((t) < 4) ? l2_type_names[(t)] : "Unk")

  if (state->valid[H16_L2_START / 4]) {
    uint32_t val = state->values[H16_L2_START / 4];
    printf("        L2_Control: 0x%08x (src1_relu: %d, padding: %d, src2_relu: %d, barrier_en: %d, "
           "barrier_idx: %d)\n",
           val, val & 1, (val >> 2) & 3, (val >> 4) & 1, (val >> 16) & 1, (val >> 17) & 0x7f);
  }
  ...
  if (state->valid[(H16_L2_START + 0x04) / 4]) {
    ...
    printf("        Src1Cfg  : Type=%u (%s) Dependent=%u EnRelu=%u DMAFmt=%u (%s) "
           ...
           l2->src1_cfg.src_type, L2_TYPE_STR(l2->src1_cfg.src_type), l2->src1_cfg.dependent,
           l2->l2_control.src1_relu, dma_fmt, fmt_str, l2->src1_cfg.alias_conv_src,
           l2->src1_cfg.alias_planar_src, l2->src1_cfg.alias_conv_rslt,
           l2->src1_cfg.alias_planar_rslt, l2->src1_cfg.compression);
  }
  ...
  if (state->valid[(H16_L2_START + 0x08) / 4]) {
    ...
    printf("        Src2Cfg  : Type=%u (%s) Dependent=%u EnRelu=%u DMAFmt=%u (%s) "
           "Alias(C=%d,P=%d,CR=%d,PR=%d) Cmp=%u\n",
           l2->src2_cfg.src_type, L2_TYPE_STR(l2->src2_cfg.src_type), l2->src2_cfg.dependent,
           l2->l2_control.src2_relu, dma_fmt, fmt_str, l2->src2_cfg.alias_conv_src,
           l2->src2_cfg.alias_planar_src, l2->src2_cfg.alias_conv_rslt,
           l2->src2_cfg.alias_planar_rslt, l2->src2_cfg.compression);
  }
  ...
  if (state->valid[(H16_L2_START + 0x0c) / 4]) {
    ...
    printf("        L2_SrcIdxCfg: Type=%u (%s) Dep=%u DMAFmt=%u (%s) AliasConv(S=%d,R=%d)\n",
           l2->srcidx_cfg.src_type, L2_TYPE_STR(l2->srcidx_cfg.src_type), l2->srcidx_cfg.dependent,
           l2->srcidx_cfg.dma_fmt, fmt_str, l2->srcidx_cfg.alias_conv_src,
           l2->srcidx_cfg.alias_conv_rslt);
    printf("                      AliasPlanar(S=%d,R=%d) Bit27=%u\n",
           l2->srcidx_cfg.alias_planar_src, l2->srcidx_cfg.alias_planar_rslt, l2->srcidx_cfg.bit27);
  }
  ...
  if (state->valid[(H16_L2_START + 0x38) / 4]) {
    ...
    printf("        L2_SrcIdxStrides: C=0x%05x0 D=0x%05x0 G=0x%05x0\n", l2->srcidx.channel_stride,
           l2->srcidx.depth_stride, l2->srcidx.group_stride);
  }
  ...
  if (state->valid[(H16_L2_START + 0x48) / 4]) {
    ...
    printf("        L2_ResultCfg: Type=%u (%s) DMAFmt=%u (%s) Intrlv=%u Cmp=%u\n",
           l2->result_cfg.res_type, L2_TYPE_STR(l2->result_cfg.res_type), l2->result_cfg.dma_fmt,
           fmt_str, l2->result_cfg.interleave, l2->result_cfg.compression);
    printf("        L2_ResultBase: 0x%05x0\n", l2->result.base);
    printf("        L2_ResultStrides: C=0x%05x0 R=0x%05x0 D=0x%05x0 G=0x%05x0\n",
           l2->result.channel_stride, l2->result.row_stride, l2->result.depth_stride,
           l2->result.group_stride);
  }

  // Dump intermediate "Res" registers if valid
  if (state->valid[(H16_L2_START + 0x60) / 4]) printf("        L2_Res24 : 0x%08x\n", l2->l2_res24);

  for (int i = 0; i < 3; i++) {
    if (state->valid[(H16_L2_START + 0x64 + i * 4) / 4]) {
      printf("        WrapCfg[%d]: Blocks=%d Len=0x%05x\n", i, l2->wrap_cfg[i].wrap_num_blocks,
             l2->wrap_cfg[i].wrap_len);
    }
  }

  if (state->valid[(H16_L2_START + 0x70) / 4]) printf("        L2_Res28 : 0x%08x\n", l2->l2_res28);

  if (state->valid[(H16_L2_START + 0x74) / 4]) {
    printf("        ResultWrap: Mask=0x%x StartOffset=0x%x\n",
           l2->result_wrap_idx_off.wrap_index_mask, l2->result_wrap_idx_off.wrap_start_offset);
  }

  if (state->valid[(H16_L2_START + 0x78) / 4]) printf("        L2_Res30 : 0x%08x\n", l2->l2_res30);
  ...
  if (state->valid[(H16_L2_START + 0x8c) / 4]) {
    printf("        PEIndex  : Trans=%d Mode=%d Broadcast=%d MaxIdx=%d\n",
           l2->pe_index_cfg.transpose, l2->pe_index_cfg.mode, l2->pe_index_cfg.broadcast,
           l2->pe_index_cfg.max_index);
  }

  if (state->valid[(H16_L2_START + 0x90) / 4]) printf("        L2_Res36 : 0x%08x\n", l2->l2_res36);
  if (state->valid[(H16_L2_START + 0x94) / 4]) printf("        L2_Res37 : 0x%08x\n", l2->l2_res37);
  if (state->valid[(H16_L2_START + 0x98) / 4]) printf("        L2_Res38 : 0x%08x\n", l2->l2_res38);

  if (state->valid[(H16_L2_START + 0x9c) / 4]) {
    printf("        ResultWrapIdx: Addr=0x%x\n", l2->wrap_addr);
  }
  if (state->valid[(H16_L2_START + 0xa0) / 4]) {
    printf("        CropTex   : S1X=%d S1Y=%d S2X=%d S2Y=%d\n", l2->crop_tex.s1x, l2->crop_tex.s1y,
           l2->crop_tex.s2x, l2->crop_tex.s2y);
  }
}
...
void print_tiledmasrc_h16(const hwx_state_t *state) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (!state->valid[base_word]) continue;

    printf("        %sDMAConfig : En=%u (%s) DSID/Hint=%u Tag=%u DepInt=%u DepMode=%u\n",
           src_names[i], src->dmacfg[i].enable, src->dmacfg[i].enable ? "Enabled" : "Disabled",
    ...
    if (state->valid[(H16_TILEDMA_SRC_START + 0x10 + (i * 0x18)) / 4]) {
      printf("        %sBase       : 0x%08x%08x\n", src_names[i], src->strides[i].base_hi,
             src->strides[i].base_lo);
      printf("        %sStrides    : Row=0x%x Chan=0x%x Depth=0x%x Group=0x%x\n", src_names[i],
             src->strides[i].row_stride, src->strides[i].plane_stride, src->strides[i].depth_stride,
    ...
    if (state->valid[(H16_TILEDMA_SRC_START + 0x68 + (i * 4)) / 4]) {
      ...
      printf("        %sFmt     : Mode=%u MemFmt=%u Trunc=%u Shift=%u -> %s\n", src_names[i], mode,
             mem_fmt, trunc, shift, get_hw_tensor_format_name_v17(mode, mem_fmt, trunc, shift));
      printf("                 Intrlv=%u OffCh=%d CmpVec=%d\n", src->fmt[i].interleave,
             src->fmt[i].offset_ch, src->fmt[i].cmp_vec);
    }

    // Compressed Info
    if (state->valid[(H16_TILEDMA_SRC_START + 0x78 + (i * 0x10)) / 4]) {
      printf("        %sComp       : En=%u (%s) PF=%u MBS=%u Lossy=%u MdTag=0x%x\n", src_names[i],
             src->compinfo[i].compressed_enable,
  ...
  if (state->valid[(H16_TILEDMA_SRC_START + 0xC8) / 4]) {
    ...
    uint32_t mode = val & 7;
    printf("        TextureCfg    : 0x%08x Mode=%u (%s) Norm1=%u Norm2=%u Filter=%u BGEn=%u "
           "DepthVal=%u Wrap=%u\n",
           val, mode, get_texture_mode_name(mode), (val >> 3) & 7, (val >> 6) & 7, (val >> 12) & 7,
...
void print_tiledmadst_h16(const hwx_state_t *state) {
  ...
  if (state->valid[H16_TILEDMA_DST_START / 4]) {
    printf("        DstDMAConfig: En=%d (%s) DSID=%u Tag=%u\n", dst->dstcfg.en,
           dst->dstcfg.en ? "Enabled" : "Disabled", dst->dstcfg.dataset_id, dst->dstcfg.user_tag);
  }
  if (state->valid[(H16_TILEDMA_DST_START + 0x10) / 4]) {
    printf("        DstStrides: Row=0x%08x Plane=0x%08x Depth=0x%08x "
           "Group=0x%08x\n",
           dst->dstrow_stride, dst->dstplane_stride, dst->dstdepth_stride, dst->dstgroup_stride);
  }
  if (state->valid[(H16_TILEDMA_DST_START + 0x28) / 4]) {
    ...
    printf("        DstMeta   : Addr=0x%x%08x FmtMode=%u (%s) Size=0x%x\n", dst->dstmeta_hi,
           dst->dstmeta_lo, fmt_mode, get_hw_tensor_format_mode_name(fmt_mode),
  ...
  if (state->valid[(H16_TILEDMA_DST_START + 0x38) / 4]) {
    ...
    printf("        DstFmt: Mode=%u MemFmt=%u Trunc=%u Shift=%u -> %s\n", mode, mem_fmt, trunc,
           shift, get_hw_tensor_format_name_v17(mode, mem_fmt, trunc, shift));
    printf("                OffCh=%u ZeroPad (F=%u, L=%u) Intrlv=%u CmpVec=%u\n",
           dst->dstfmt.offset_ch, dst->dstfmt.zero_pad_first, dst->dstfmt.zero_pad_last,
           dst->dstfmt.interleave, dst->dstfmt.cmp_vec);
  }
...
void print_kerneldmasrc_h16(const hwx_state_t *state) {
  ...
  if (state->valid[H16_KERNELDMA_START / 4])
    printf("        MasterCfg: En=%d (%s) Sparse=%d Reuse=%d\n", k->master_cfg.master_enable,
           k->master_cfg.master_enable ? "Enabled" : "Disabled", k->master_cfg.kernel_sparse_fmt,
  ...
  for (int i = 0; i < 16; i++) {
    if (state->valid[(H16_KERNELDMA_START + 0x20) / 4 + i]) {
      printf("        CoeffCfg[%d] : En=%d (%s) DSID=%u Tag=%u\n", i, k->coeff_cfg[i].en,
             k->coeff_cfg[i].en ? "Enabled" : "Disabled", k->coeff_cfg[i].dataset_id,
...
void print_kerneldmasrc_h17(const hwx_state_t *state) {
  ...
  if (state->valid[H16_KERNELDMA_START / 4])
    printf("        MasterCfg: En=%d Sparse=%d Reuse=%d\n", k->pe_cfg.enable, k->pe_cfg.sparse,
           k->pe_cfg.reuse);
  if (state->valid[(H16_KERNELDMA_START + 0x04) / 4])
    printf("        AlignedCoeffSize: 0x%08x\n", k->aligned_coeff_size);
  if (state->valid[(H16_KERNELDMA_START + 0x08) / 4])
    printf("        Prefetch : 0x%08x\n", k->prefetch);
  if (state->valid[(H16_KERNELDMA_START + 0x18) / 4]) printf("        StrideX  : %u\n", k->stridex);
  if (state->valid[(H16_KERNELDMA_START + 0x1c) / 4]) printf("        StrideY  : %u\n", k->stridey);

  for (int i = 0; i < 16; i++) {
    if (state->valid[(H16_KERNELDMA_START + 0x20) / 4 + i]) {
      printf("        CoeffCfg[%d] : Hint=%u DSID=%u Tag=%u\n", i, k->coeff_dma_cfg[i].cache_hint,
             k->coeff_dma_cfg[i].dataset_id, k->coeff_dma_cfg[i].user_tag);
    }
  }
  for (int i = 0; i < 16; i++) {
    if (state->valid[(H16_KERNELDMA_START + 0x60) / 4 + i]) {
...
void print_cachedma_h16(const hwx_state_t *state) {
  ane_cachedma_h16_t cdma = *(ane_cachedma_h16_t *)&state->values[H16_CACHEDMA_START / 4];
  printf("        --- CacheDMASrc (0x5900) ---\n");
  if (state->valid[H16_CACHEDMA_START / 4]) {
    printf("        Control: Flush=%d En=%d (%s) TaskSync=0x%x ET=0x%x FL=%d "
           "Thresh=0x%04x\n",
           cdma.control.flush, cdma.control.enable, cdma.control.enable ? "Enabled" : "Disabled",
           cdma.control.task_sync, cdma.control.early_term, cdma.control.footprint_limiter,
...
void report_hwx_state_json(const hwx_state_t *state) {
  ...
  for (uint32_t r = 0; r < HW_MAX_REGS; r++) {
    if (state->valid[r]) {
      ...
      [reg setObject:[NSString stringWithFormat:@"0x%08x", state->values[r]] forKey:@"val"];
      if (name) [reg setObject:[NSString stringWithUTF8String:name] forKey:@"name"];
      [regs addObject:reg];
    }
  }
...
void decode_ane_td(const uint8_t *ptr, size_t total_len, uint32_t subtype, BOOL dump_reg_blocks,
                   BOOL dump_json) {
  uint32_t offset = 0;
  int task_idx = 0;
  ...
  while (offset + sizeof(ane_header_h13_t) <= total_len) {
    ...
    if (offset + sizeof(ane_header_h13_t) <= total_len) {
      ...
      uint32_t max_payload_bytes = (total_len > offset + sizeof(ane_header_h13_t))
      ...
      if (td->next_pointer > offset + sizeof(ane_header_h13_t)) {
        uint32_t td_words = (td->next_pointer - offset - sizeof(ane_header_h13_t)) / 4;
...
void decode_ane_td_m4(const uint8_t *ptr, size_t total_len, uint32_t subtype, BOOL dump_reg_blocks,
                      BOOL dump_json) {
...
static void handle_segment_64(const struct mach_header_64 *header, const struct load_command *lc,
                              NSData *data, BOOL dump_hexdump, BOOL dump_reg_blocks,
                              BOOL dump_json) {
  ...
  const struct section_64 *sect =
      (const struct section_64 *)((const uint8_t *)lc + sizeof(struct segment_command_64));
  for (uint32_t j = 0; j < seg->nsects; j++) {
    if ((const uint8_t *)(sect + 1) > (const uint8_t *)data.bytes + data.length) break;
    printf("    Section %u:\n", j);
    printf("      Name: %s\n", sect->sectname);
    printf("      Segment: %s\n", sect->segname);
    printf("      Addr: 0x%llx\n", sect->addr);
    ...
    if (strcmp(seg->segname, "__TEXT") == 0) {
      if (sect->offset + sect->size <= data.length) {
        ...
        if (strcmp(sect->sectname, "__text") == 0 || strcmp(sect->sectname, "__TEXT") == 0) {
          ...
          if (instr_ver >= 11) {
            decode_ane_td_m4(section_ptr, section_size, header->cpusubtype, dump_reg_blocks,
                             dump_json);
          } else {
            decode_ane_td(section_ptr, section_size, header->cpusubtype, dump_reg_blocks,
                          dump_json);
          }
          if (dump_hexdump) {
...
static void handle_symtab(const struct load_command *lc, NSData *data, BOOL dump_all_symbols) {
  ...
  if (sym->nsyms > 0 && sym->symoff < data.length) {
    ...
    for (uint32_t k = 0; k < max_syms; k++) {
      if ((const uint8_t *)(list + 1) > (const uint8_t *)data.bytes + data.length) break;
      const char *name = "";
...
static void handle_note(const struct load_command *lc, NSData *data) {
  if (lc->cmdsize < sizeof(struct note_command)) return;
...
void print_macho_headers(NSData *data, BOOL dump_all_symbols, BOOL dump_threads, BOOL dump_hexdump,
...
int main(int argc, char *const argv[]) {
  @autoreleasepool {
    ...
    print_macho_headers(data, dump_all, dump_threads, dump_hexdump, dump_reg_blocks, dump_json);
  }
  return 0;
}
