...
def parse_dump(file_path, sym_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            m = re.search(r'(str|strb|strh|ldr|ldrb|ldrh)\s+w\d+,\s*\[(x\d+|x19),\s*#0x([0-9a-fA-F]+)\]', instr)
            if m:
                op = m.group(1)
                offset_hex = m.group(3)
                offset_val = int(offset_hex, 16)
                
                # Filter bounds relevant to TileDMA offsets in M4 object
                if 0x250 <= offset_val <= 0x51c and op.startswith('str'):
...
if __name__ == '__main__':
    parse_dump('tiledma_block_dump.txt', 'all_tiledma_setters.txt')
